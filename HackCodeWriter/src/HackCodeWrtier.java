...
public class HackCodeWrtier
{
	...
	static ArrayList<Symbol> classSymbolTable = new ArrayList<Symbol>(),
			subSymbolTable = new ArrayList<Symbol>();
	...
	public static String classMethod(Token[] token)
	{
		...
		for(Symbol s : classSymbolTable)
			System.out.println(s.getName() + "::" + s.getType() + "::" + s.getKind());
	...
	public static String classVarDec(Token[] token)
	{
		...
		retString = retString.concat(token[index].toString());
		index++;
		tempSymbol = new Symbol(token[index].getName(), token[index - 1].getName(), token[index - 2].getName());
		classSymbolTable.add(tempSymbol);
		retString = retString.concat(token[index].toString());
		index++;
		...
		if(token[index].getName().equals(","))
		{
			while(!token[index].getName().equals(";"))
			{
				if(!token[index].getName().equals(","))
				{
					tempSymbol = new Symbol(token[index].getName(),
							classSymbolTable.get(classSymbolTable.size() - 1).getType(),
							classSymbolTable.get(classSymbolTable.size() - 1).getKind());
					classSymbolTable.add(tempSymbol);
				}
				retString = retString.concat(token[index].toString());
				index++;
	...
	public static String parameterList(Token[] token)
	{
		...
		while(!token[index].getName().equals(")"))
		{
			...
			retString = retString.concat(token[index].toString());
			index++;
			tempSymbol = new Symbol(token[index].getName(), token[index - 1].getName(), "arg");
			subSymbolTable.add(tempSymbol);
			...
			if(token[index].getName().equals(","))
			{
				if(!token[index].getName().equals(",") && !token[index - 1].getName().equals(","))
				{
					tempSymbol = new Symbol(token[index].getName(),
							subSymbolTable.get(subSymbolTable.size() - 1).getType(),
							subSymbolTable.get(subSymbolTable.size() - 1).getKind());
					subSymbolTable.add(tempSymbol);
				}
				retString = retString.concat(token[index].toString());
				index++;
	...
	public static String subroutineCall(Token[] token)
	{
		...
		{
			System.out.println(token[index + 2].getName());
